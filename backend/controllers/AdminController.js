...
const AdminController = {
    ...
    createExam: async (req, res) => {
        const {name, type, question, img, answer_a, answer_b, answer_c, answer_d, answer_true} = req.body

        if(!name|| !type || !question || !answer_a || !answer_b || !answer_c || !answer_d || !answer_true)
            return res.status(400).json({success: false, message: 'Tạo thất bại'})

        try {
            const newExam = new Exam({ name, type, question, img, answer_a, answer_b, answer_c, answer_d, answer_true })
